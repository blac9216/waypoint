...
| `master.key` | `backend`, `compliance-runner`, `download-runner` (envelope encryption, ADR-0005) | production and `--mode persistent`: operator/developer-supplied, never generated (see "Optional: secrets master key" below); `--mode agent`: generated automatically, since a throwaway agent stack has no operator to supply one |
...
#### Optional: secrets master key (issue #405)

Production and persistent-dev stacks come up healthy without a master key —
only the credential store depends on it. `POST /api/v1/credentials` (or any
other secret-bearing write) answers `503 master_key_unavailable`
(`docs/api-contract.md`'s error-code table) until one is configured; agent-mode
stacks generate one automatically and need no manual step here.

```bash
mkdir -p config/secrets
openssl rand -hex 32 > config/secrets/master.key
chmod 644 config/secrets/master.key
```

Never commit this file (`deploy/config/` is gitignored) — losing it after real
credentials are encrypted with it makes them permanently unrecoverable. Then
uncomment the `waypoint-master-key` bind mount on all three trusted services
(`backend`, `compliance-runner`, `download-runner`) in `compose.yaml`, and
restart them (`FileMasterKeyProvider` caches a missing-key failure for the
process lifetime, so fixing the file alone does not take effect until
`docker compose restart backend compliance-runner download-runner`). `644` is
the convention here because a Compose `file:`/bind-mount secret is
bind-mounted verbatim and all three images run as uid `1654`; use
`chown 1654 config/secrets/master.key && chmod 600` instead if you prefer
tighter host permissions.
