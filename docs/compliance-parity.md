...
## Normalized supported-component matrix
...
| Product version | Kind / exact profile version | Components | Transport / selector | Purpose | Output |
|---|---|---|---|---|---|
| vSphere 8.0 | STIG / `v2r3-stig` | vCenter; ESXi; VM | `vmware` / object kind | `vsphere-api` | HDF + CKL |
| vSphere 8.0 | STIG / `v2r3-stig` | VCSA EAM, Lookup, PerfCharts, Photon, PostgreSQL, STS, UI, VAMI, Envoy | `ssh` / named VCSA service | `vsphere-api` + `vcsa-ssh` | HDF + CKL |
| vSphere 9.0 | SRG / `Y26M05-srg` | vCenter; ESXi; VM | `vmware` / object kind | `vsphere-api` | HDF |
| vSphere 9.0 | SRG / `Y26M05-srg` | VCSA Envoy, PostgreSQL, VAMI, Photon | `ssh` / named VCSA service | `vsphere-api` + `vcsa-ssh` | HDF |
| NSX 4.x | STIG / `v1r2-stig` | Manager, distributed firewall, tier-0 firewall, tier-0 router, tier-1 firewall, tier-1 router | `nsx-api` / named function | `nsx-api` | HDF + CKL |
| NSX 9.x | SRG / `Y26M05-srg` | Manager, routing | `nsx-api` / named function | `nsx-api` | HDF |
| Aria Operations 8.x | SRG / `v1r4-srg` | Aria Operations | `ssh` / target | `srg-ssh` | HDF |
| Aria Automation 8.x | SRG / `v1r6-srg` | Aria Automation | `ssh` / target | `srg-ssh` | HDF |
| Aria Suite Lifecycle 8.x | SRG / `v1r2-srg` | Aria Suite Lifecycle | `ssh` / target | `srg-ssh` | HDF |
| Workspace ONE Access 3.3.x | SRG / `v1r3-srg` | Workspace ONE Access | `ssh` / target | `srg-ssh` | HDF |
| Photon OS 5.0 | SRG / `v3r3-srg` | Photon OS | `ssh` / target | `srg-ssh` | HDF |
| VCF 9.x | SRG / `Y26M05-srg` | SDDC Manager nginx, PostgreSQL, Photon; Operations httpd, PostgreSQL, Photon; Operations HCX httpd, Photon; Operations Networks nginx platform, Ubuntu | `ssh` / named service | `srg-ssh` | HDF |
| VCF 9.x | SRG / `Y26M05-srg` | SDDC Manager application; Automation application | `vcf-api` / named service | catalog-declared API purpose (#807) | HDF |

Every row maps one exact product version to one exact profile version. Version ranges,
nearest-version selection, cross-version evidence, and a scan-time profile picker are
prohibited. Exact XCCDF identities and mappings for each STIG component are baseline
data staged under ADR-0022, not hard-coded path inference.
