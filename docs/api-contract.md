...
- `unsafe_selector_name` — issue #1138: a narrowable `esxi`/`vm` component's `DisplayName` contains a character outside the conservative allow-list `[A-Za-z0-9._-]`. The vendor ESX baseline content interpolates the value unquoted into its PowerCLI object selector, so PowerCLI wildcards (`*` `?` `[` `]`, which silently widen `-Name` to more than one object), PowerShell metacharacters (`` ` `` `$` `;` `|` `&` `(` `)` `{` `}` `<` `>` `'` `"` `#` `,` `=` `^` `!` `%` `~`), whitespace, and control/non-ASCII characters are all refused. The skip `detail` names the offending character class.
