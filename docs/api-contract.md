...
- `ambiguous_selector_name` — issue #1138: two or more narrowable `esxi`/`vm` components on the same vSphere target (vCenter) share a `DisplayName`; the vendor content's name-based `Get-VMHost`/`Get-VM` lookup would resolve to more than one object, so every colliding component is omitted rather than scanned ambiguously.
- `unsafe_selector_name` — issue #1138: a narrowable `esxi`/`vm` component's `DisplayName` contains whitespace or a quote character, which the vendor ESX baseline content interpolates unquoted into its PowerCLI object selector.
