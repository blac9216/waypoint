...
`counts_available` is `false`) — controls whose only non-passed/skipped/not_applicable
result is `error`. Before this fix, `HdfSeverityCounter` counted an errored control as
**open** (inflating `cat_i_open`/`cat_ii_open`/`cat_iii_open`) while the persisted
`component_results` rollup counted the same control as `execution_error`, landing in
no column at all — the same control disagreed with itself across the two endpoints.
Both surfaces now agree: an errored control is **not** open (it never produced a
genuine compliance verdict) and is instead counted here, matching
`ComponentFindingStatuses.IsOpen`'s `failed`-only definition exactly. A control with
both an `error` and a `failed` result counts as execution-error, not open — the same
worst-of priority `HdfFindingsParser.MapStatus` already used.
