...
- `benchmark_missing` — the component's bound kind is `stig` and its CURRENT mapping has no benchmark revision (status `unmapped`/`suggested`/`ambiguous`, or no mapping decision has ever been recorded). A persistent, honest, **non-blocking** open alert: it does not block baseline activation/approval of the profile-only baseline, and does not block scanning any *other* component — the affected component itself is skipped at scan-plan time only (`ScanPlanSkipReasons.unmapped_benchmark`, pre-existing, unchanged by this issue) until an XCCDF is mapped and the baseline is re-approved.
