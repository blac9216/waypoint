...
| Content-library **organize** (folder create/rename/move, item→folder assignment) | — | ✅ | ✅ | `ContentLibraryFoldersController.Create`/`Update`/`AssignItem` (PR #1770, closes #1389/#1746) | ✅ yes — R2-10 names "library upload/organize" as Operator-tier; shipped as Operator+. |
| Content-library **folder delete** | — | — | ✅ | `ContentLibraryFoldersController.Delete` (PR #1770) | Admin-only per the orchestrator's amendment to #1746's AC1 (delete grouped with the "deletes/purges" Admin bucket, not the organize verbs). |
...
| Consumer view (platform-set) read/write | — | — | ✅ | `ConsumerViewsController` (all verbs, including read) | ✅ yes — every verb Admin-only, including read, under R2-10's "serving/auth dials" bucket (same stricter-than-usual read floor as `RepoCredentialsController`). |
...
(`docs/explanation/security.md`, issue #521) reuses the same redirect with `prompt=login`: a
...
| Endpoint | Methods | Notes |
|---|---|---|
| `/targets/{id}/components` | GET, POST | GET: superset of `/targets/{id}/inventory`'s planned shape: every known component beneath this target regardless of lifecycle, for Configuration-screen visibility (vs. the scan-scoped view below). POST (Admin, issue #743): declares the ROOT component for a target kind with no discovery operation — today only `ssh` (whole-appliance SRG products). Body: `{ catalog_component_key, exact_version? }`. The key is validated fail-closed against the catalog's closed `ssh`/`target` shape (400 `unknown_catalog_component_key` otherwise — generic SSH never guesses a product); `exact_version`, when supplied, flows through the same configured-fact/linkage path as `PUT /components/{id}`. The row's `display_name` is the catalog component key (version-neutral) until it links to one exact catalog product version, at which point GET/read reflects that linked component's own display name (issue #1202: never a stale or arbitrarily-picked sibling version's name). 400 `declared_component_unsupported_target_kind` for discoverable kinds; 409 `component_exists` when the key is already declared under the target. |
| `/components/{id}` | GET, PUT | GET: full component record incl. observation history summary — `catalog_component_id`/`display_name` reflect the current linkage outcome, but carry no reason field for an unlinked one. PUT (Admin): `configured_fact` only (exact product version/capability Waypoint cannot discover) — never lifecycle or identity, which are discovery/refresh-owned. Setting `exact_version` resolves catalog linkage at write time through the shared linkage resolver (issue #1000; same exact-match / ambiguity-fail-closed semantics as discovery), but this endpoint exposes no linkage-reason field: it silently leaves the component unlinked when resolution fails, the same as a `PUT` that never sets `exact_version` at all. An empty or whitespace `exact_version` is an explicit CLEAR (not a 400): the configured fact is removed and linkage honestly re-resolves from whatever facts remain — unlinking when nothing remains. Issue #1082/#1254: the closed vocabulary of unlinked reasons (`no_exact_version_fact`, `out_of_declared_scope`, `ambiguous`, `lookup_failed`) is discovery's contract, not this endpoint's — see the `/targets/{id}/discover` row above for where it is published and surfaced (`job.log`, `discover.progress`). |
...
| `/runs/{id}` | GET | `RunResponse` (issue #494, matches shipped `RunsController`/`RunContracts.cs` exactly): `id`, `run_type`, `state`, `paused`, `blocked`, `blocked_reason`, `scope`, `credential_id`, `initiated_by`, `schedule_id` (issue #515 — the schedule that dispatched this run, null for an operator-initiated one; distinct from `schedules.last_run_id`, which only ever points at a schedule's most recent run), `created_at`/`started_at`/`completed_at`, and job counts by state (`job_count`, `job_count_queued`, `job_count_running`, `job_count_completed`, `job_count_failed`, `job_count_blocked`) — every job on the run lands in exactly one of the five buckets (issue #970: `done` AND `uploaded` both count as `job_count_completed`, `failed`/`auth-failed`/`cancelled` all count as `job_count_failed`, and `running`/`attesting`/`converting` all count as `job_count_running`), so `job_count_queued + job_count_running + job_count_completed + job_count_failed + job_count_blocked == job_count` always holds. Issue #1140: `coverage_incomplete` (bool) mirrors `GET /runs/{id}/component-results/summary`'s own flag exactly (no recorded scan plan, a plan-time coverage omission, or at least one component that evaluated zero controls) -- computed in bulk in the shared run projection query (`JobQueueRepository.RunSummaryProjectionSql`), so `GET /runs`, `GET /runs/{id}`, and `GET /runs/history` all carry the same signal with no per-run follow-up query. No per-queue/per-benchmark breakdown and no aggregate `pass`/`fail`/`na` — those live only on `/runs/{id}/artifacts`. `blocked`/`blocked_reason` are the run's single credential-halt flag (ADR-0008), not a list of independently-blockable named queues; the frontend (`liverun.ts`) synthesizes a queue-like grouping client-side from each job's `priority` for display, it is not a server concept. |
...
| `/runs/{id}/purge` | POST, GET | Issue #594 (epic #577): Admin-only, crash-safe purge of a **terminal** compliance run's owned database projections and artifact files. `POST` body: `{ confirmation: "PURGE" }` (400 otherwise — never implicit, same step-up shape as remediate's `confirmation: "REMEDIATE"`). 409 `run_not_terminal` when `state` is not `completed`/`completed_with_failures`/`aborted` (run left untouched); 404 when the run does not exist. **Design: `runs`/`jobs` rows are retained, never deleted** — job_events is append-only-by-trigger and FK'd to `jobs`, so deleting the owning row would either corrupt or require deleting that immutable SSE ledger too; instead `runs.purged_at` marks the run purged in place and a `run_purge_tombstones` row is the durable historical record. What purge actually removes: `attestation_snapshots` rows for the run (a narrow, GUC-gated exception to that table's own append-only trigger — see migration 0042), any leftover `run_secrets` row (normally already gone via the run's own terminal transition), the scan-artifact HDF/CKL files for every `scan` job in the run (deleted by a `purge` job the compliance-runner executes against its own read-write artifact mount — the API process mounts that volume read-only and cannot delete a file itself), and nulls any `schedules.last_run_id` pointing at the run. 200/202 body (`RunPurgeStatusResponse`): `run_id`, `outcome` (`Completed`\|`AlreadyPurged`\|`InProgress`\|`Failed`), `requested_by`, `requested_at`, `prior_state`, `db_phase_done`, `artifacts_phase` (`pending`\|`running`\|`done`\|`failed`), `artifacts_total`, `artifacts_deleted`, `last_error`, `completed_at`. **`artifacts_total`/`artifacts_deleted` count artifact *files*, in every phase (issue #1240):** the scan-artifact HDF/CKL files `PurgeJobHandler` attempts to delete (`Waypoint.Core.Scans.ScanArtifactPaths.FilesPerJob`, 3, per `scan` job in the run — raw HDF, attested HDF, CKL — whether or not each one actually exists on disk, since a missing file is a tolerated no-op, not a skip), never the scan-job count. `RunPurgeService` computes this file total at enqueue time (`scanJobIds.Count * FilesPerJob`) — the same unit the handler enumerates and deletes — so an in-flight poll (`artifacts_phase: "running"`) and the completed poll report the same number for the same run; before #1240 the in-flight total was the scan-**job** count, which changed unit (and value) the moment the purge completed. **Issue #1061:** once `outcome` is `Completed`, `artifacts_total`/`artifacts_deleted` are read back from the `run_purge_tombstones.detail` JSON (`{"artifacts_deleted": N}`) rather than from the in-flight `run_purges` row — the tombstone does not separately carry a distinct total (nothing was left over once a purge reaches `completed`), so `artifacts_total` is reported equal to `artifacts_deleted` for a completed purge. A tombstone whose detail is missing, non-numeric, fractional/out-of-range, or negative reports both as `0` (logged as a warning distinguishing which case) rather than throwing or surfacing a fabricated count. Idempotent and retryable: calling `POST` again on an in-flight purge resumes it (the database phase is never redone; the artifact job is only re-enqueued if the prior attempt's `artifacts_phase` is `failed`), and calling it again on an already-completed purge returns `AlreadyPurged` with the original tombstone's `requested_by`/`prior_state` rather than erroring or double-writing. `GET` polls the same status shape without re-triggering anything; 404 if purge was never requested for this run. Never schedulable (`purge` is absent from the closed schedule `job_type` set, `ScheduleJobTypes.All`) — mirrors `remediate`'s exclusion. |
...
| `/runs/{id}/artifacts` · `/jobs/{id}/artifacts/{kind}` | GET | Per-target rows + CKL/HDF download; `?bundle=zip` for the export button. Row CAT counts are **nullable** and gated by `counts_available`, and (issue #1132/#1144) carry an evaluated-control denominator and an execution-error count — see below. |
...
| `/runs/{id}/plan` | GET | Issue #1125: the frozen scan plan for an already-created run — `plan_schema_version`, `plan_digest`, `explanation`, `accepted_component_count`, and `skips` (one row per plan-time coverage omission: `component_id`, `reason`, `detail`, verbatim from `scan_plans.skips_json`). 404 for a run with no recorded plan (predates issue #734, or a legacy request shape with no `target_scope`) — never a zeroed/empty plan. Same data `POST /runs/plan-preview` shows before creation, now readable afterwards. Issue #1082: `explanation` distinguishes three situations that previously collapsed into one string whenever the resolved-component count was zero — nothing was requested at all ("No components were requested..."), something was requested but every candidate was omitted during scope resolution before reaching the planner ("N component(s) requested; all N omitted during scope resolution (reason counts); none reached the execution planner"), and a partial omission where some candidates were omitted and the rest planned normally (both the omission and planning outcome named together). A fourth case (round-2 review of PR #1232): when no component was requested but the requested SCOPE itself failed to resolve — e.g. a `target_not_found` omission, which names no component and therefore contributes to no component count — the plan is never called "intentionally empty"; `explanation` reads "No components were requested; N requested target(s) could not be resolved (reason counts); this plan is empty because the requested scope could not be resolved, not by intent." so it can never contradict a non-empty `scope_omissions`. Round-2 review of PR #1232 generalised that to an invariant over EVERY case: the component-level and target-level omission clauses are each appended whenever non-empty, on every wording branch, so the all-clear phrasings ("no gaps found", "intentionally empty") appear only when `scope_omissions` is entirely empty — a plan that is runnable but lost a whole target reads "N of N requested component(s) accepted into the plan; 1 requested target(s) could not be resolved (1 target_not_found); no further gaps among the resolved components." |
| `/runs/{id}/component-results/summary` | GET | Issue #745's per-status rollup, extended (issue #1125/#1132/#1144). `requested_component_count`/`omitted_component_count` fold the plan's frozen skips in alongside the already-planned `planned_component_count`. Each `by_status` row now carries `execution_error_count` (migration 0080): the sum of findings mapped to `execution_error` for that bucket's latest-attempt components — previously counted in no column at all, which made an all-errored component read completely all-zero. `execution_error_count` is deliberately **not** folded into `cat_i_open`/`cat_ii_open`/`cat_iii_open` — an errored control never produced a genuine compliance verdict, so it is not "open", matching `ComponentFindingStatuses.IsOpen`'s `failed`-only definition. This bucket is not only literal `status: "error"`: `HdfFindingsParser` and `HdfSeverityCounter` share ONE classification rule (`HdfControlClassifier.Classify`), so an unrecognized status string and an unrecognized mixed result shape (e.g. `passed` + `skipped`) are `execution_error` on **both** this endpoint and `/runs/{id}/artifacts` — no residual divergence in the *classification* of any control both surfaces see. One documented divergence in the *control set* remains: a control with no `id` is dropped by `HdfFindingsParser` (no identity to persist a finding under) but still counted by `HdfSeverityCounter`, so such a control lands in `controls_execution_error` on `/runs/{id}/artifacts` and in nothing here — the same asymmetry `controls_total` already has, malformed-input-only (see the `/runs/{id}/artifacts` reconciliation below). Migration 0080 **backfills** this column for rows recorded before it ran, deriving each row's value from its own immutable `component_result_findings` — a pre-0080 all-errored component reads its real count, not a fabricated `0`. `evaluated_zero_component_count` (run-level, and per `by_status` bucket) counts components that reported a result but produced **no verdict** — zero passed and zero open findings; it is counted **per component** in SQL, not derived from a bucket's summed counts, so a mixed bucket (one component that evaluated nothing, others evaluated normally, healthy-looking sums) still reports it. Counted shapes: all-`not_reviewed`, all-`skipped`, all-`execution_error`, and a component that produced no findings at all. The per-bucket bool `evaluated_zero_controls` is just `evaluated_zero_component_count > 0`. A component that is genuinely all-`not_applicable` is never counted — N/A is a determinate outcome, not a failure to evaluate. Issue #1144 closed the one gap this flag used to have: a component mixing `not_applicable` with only `execution_error` findings used to be indistinguishable from a genuine all-N/A component; `execution_error_count > 0` is now checked directly rather than inferred from `not_applicable_count = 0`, so that mixed shape is correctly flagged. `coverage_incomplete` (bool) is true when **any** of: the plan omitted a requested component, `evaluated_zero_component_count > 0`, or no plan was recorded. `false` therefore means "a plan was recorded, it omitted nothing, and no component produced a zero-verdict result". `plan_recorded` (bool) is false for a run with no frozen `scan_plans` row (predates issue #734, or a legacy request shape — exactly the runs `/runs/{id}/plan` 404s for); there is then no requested-component set to compare delivered coverage against, so `requested_component_count`/`omitted_component_count` are null and therefore **absent** from the JSON (the server omits null properties) rather than a misleading `0`, and `coverage_incomplete` is `true` — unknown coverage is not a completeness claim. **Not folded into `coverage_incomplete`:** plan items with no `component_results` row at all (`planned_component_count` > the sum of `by_status[].component_count`) — components still queued/running, or never claimed. A caller that needs "did every planned component report?" MUST compare those two numbers itself; the flag covers plan-time omissions, evaluated-nothing components, and unknown plans only. See below. |
...
#### `/runs/{id}/plan` skip reason closed vocabulary

`scan_plans.skips_json[].reason` (surfaced by both `/runs/{id}/plan` and
`POST /runs/plan-preview`) is a closed, application-enforced set
(`ScanPlanSkipReasons.All` — not a database CHECK constraint; `scan_plans.skips_json`
is JSONB, pinned by a drift test instead):

- `unsupported` — no catalog-compatible execution profile.
- `no_active_baseline` — a compatible profile exists but has no active, approved baseline.
- `missing_required_input` — a required declared Input resolved to no config document at any layer.
- `credentialed_transport_with_no_requirement` — a credentialed transport's profile carries zero `catalog_credential_requirements` rows (defense-in-depth, issue #1012).
- `ambiguous_selector_name` — issue #1138: two or more narrowable components **of the same `selector_kind`** (`esxi` or `vm`) on the same vSphere target (vCenter) share a `DisplayName` (compared case-insensitively); the vendor content's name-based `Get-VMHost`/`Get-VM` lookup would resolve to more than one object, so every colliding component is omitted rather than scanned ambiguously. An `esxi` host and a `vm` sharing a name are *not* a collision — different cmdlets resolve them — so the group key is the triple (parent target, selector kind, `DisplayName`).
- `unsafe_selector_name` — issue #1138: a narrowable `esxi`/`vm` component's `DisplayName` contains a character that is unsafe for that selector kind. The rule is per kind, because the vendored dod-compliance-and-automation content quotes the two kinds differently (measured over vSphere 7.0 + 8.0): the **ESX** baselines interpolate the value **unquoted** (`Get-VMHost -Name #{vmhostName}` — 740 files, 6 quoted), while the **VM** baselines interpolate it into a **single-quoted** literal (`Get-VM -Name '#{vmName}'` — 277 files, 0 unquoted). For `esxi` the conservative allow-list `[A-Za-z0-9._-]` therefore applies: PowerCLI wildcards (`*` `?` `[` `]`, which silently widen `-Name` to more than one object), PowerShell metacharacters (`` ` `` `$` `;` `|` `&` `(` `)` `{` `}` `<` `>` `'` `"` `#` `,` `=` `^` `!` `%` `~`), whitespace, and control/non-ASCII characters are all refused (ESXi host names are FQDNs, so this costs no realistic coverage). For `vm` spaces and ordinary punctuation are **accepted** — they are literal inside single quotes — and only `'`, the wildcards `*` `?` `[` `]`, control characters and non-ASCII are refused; non-ASCII is refused conservatively because Waypoint cannot prove the vendor input file's and the remote PowerShell host's encodings round-trip the bytes (durable fix tracked by #1137). The skip `detail` names the offending character class and the kind-specific rule.

`unmapped_benchmark` is retired (issue #1021): no code path produces it anymore, but
historical `scan_plans.skips_json` rows written before the fix still carry it, so
readers must still recognize it, not reject it as unknown.
...
**Issue #1144 — reconciled with `/runs/{id}/component-results/summary`**: the row
also carries `controls_execution_error` (nullable integer, null exactly when
`counts_available` is `false`) — controls this reader cannot turn into a genuine
compliance verdict: an `error` result, an unrecognized status string, or a mixed
result shape it does not specifically recognize. Before this fix, `HdfSeverityCounter`
counted every such control as **open** (inflating
`cat_i_open`/`cat_ii_open`/`cat_iii_open`) while the persisted `component_results`
rollup counted the same control as `execution_error`, landing in no column at all —
the same control disagreed with itself across the two endpoints.

The two surfaces now agree **exactly, by construction, for every control both of
them see**: both call the single
`HdfControlClassifier.Classify` rule (`Waypoint.Core.Scans`) — `HdfFindingsParser`
for the persisted findings, `HdfSeverityCounter` for this preview — so there is no
second copy of the classification to drift. The rule, worst-of first: any `error`
result wins; then any `failed`; then all-`passed` is passed; then all-`skipped` splits
on `impact` (`0.0` → not-applicable, anything else → not-reviewed, issue #1124); every
remaining shape — an unrecognized status string, an unrecognized mix such as
`passed` + `skipped` — is `execution_error`. Consequences stated plainly: an errored
control is **not** open (it never produced a genuine compliance verdict), it is **not**
counted in `controls_evaluated`, and **nothing unrecognized** is counted into
`cat_i_open`/`cat_ii_open`/`cat_iii_open`. A control with both an `error` and a
`failed` result counts as execution-error, not open.

**The one documented divergence — a control with no `id`.** The shared rule is the
same on both surfaces, but the *set of controls it is applied to* is not.
`HdfFindingsParser` drops a control whose `id` is missing or blank before
classifying it (there is no identity to key a `component_result_findings` row on —
the parser's one silent drop), while `HdfSeverityCounter` counts every control the
report describes, id or not, because `controls_total` is defined (issue #1132) as
the report's own denominator. So an id-less control that classifies
`execution_error` is counted in `controls_execution_error` here and contributes
nothing to `execution_error_count` on
`/runs/{id}/component-results/summary` — the same pre-existing asymmetry
`controls_total` already has against the persisted findings count. This is
malformed-input-only: real InSpec/HDF controls always carry an `id`. "Agree
exactly" above is scoped to the classification of any control both surfaces see;
it is not a promise that the two totals match for a report containing id-less
controls.
...
| `/downloads/binaries` | POST | ✅ Issue #1479/#1482 (epic #1181): the vendor-acquisition replacement for `POST /downloads` — a set of depot artifact ids, or a whole release (`product`+`version`, resolved to its member artifacts at enqueue time — decision R2-2, "whole release") → one run with one `binaries-download` job per artifact (Operator+, same scan-style run→per-item-job fanout, owner decision 18). Exactly one selection mode is required (both or neither is 400); an unknown id or empty release match is 404 before any run is created. Issue #1783: any resolved artifact with no catalog `bundle_id` (a row indexed before migration 0132, or by the offline disk walk, which has no vendor catalog document to read one from) fails the WHOLE batch with 409 `catalog_missing_bundle_id` before any run is created — re-pull the catalog to populate it. Issue #1531: before any run is created, the batch's summed catalog `size_bytes` (missing values counted as zero) is checked against the artifact store's current free bytes minus the configured disk-admission reserve (`/disk-admission-policy` below); a projected-overflow request fails the WHOLE batch with 409 `disk_admission_denied` naming the projected/free/reserve/shortfall byte counts. |
| `/downloads/readiness` | GET | ✅ Issue #560, extended by #690: combined Activation Code health + legacy Download Token health (reported independently — see `docs/explanation/security.md` — the legacy token never gates readiness) + managed-tool-installed state (Viewer+). `tool_installed` is `null` until a download-runner has heartbeated at least once. |
...
| `/downloads/esx/platforms` | GET | ✅ Issue #1470 (epic #1181): the `lcm.esx.supported.host.platforms` vendor vocabulary, read fresh at request time — never hardcoded (Viewer+). Backs the ESX patch store's VCFDT-only acquisition (ADR-0032; the original UMDS-binary install/config/EULA design from owner grill decisions 9–10 is **not built and will not be** — see `docs/explanation/security.md`). |
...
| `/disk-admission-policy` | GET, PUT | ✅ Issue #1531 (epic #1180, split from #1042): the Admin-configurable disk-admission reserve singleton (`disk_admission_policy`, migration 20260913060100, issue #1529), `{ reserve_bytes }`, mirroring `/retention-policy`'s shape exactly. `PUT` body: `{ "reserve_bytes": <non-negative integer> }` (`validation_error` on a missing/negative value). `GET`/`PUT` response: `reserve_bytes`, `updated_by` (`null` until an Admin has ever changed it), `updated_at`. Honored by the very next `POST /downloads/binaries` admission check — nothing caches the singleton row. |
| `/consumer-views` · `/consumer-views/{id}` | GET, POST, PUT, DELETE | ✅ Issue #1464 (epic #1183, split from design record #1162): named operator-defined ESX platform-set views -- `name` (unique), `platforms` (ordered, validated against the static `ConsumerViewPlatformVocabulary` list reconciled on #1156; an unknown key is 400; empty is explicitly allowed and means "all platforms" -- see the model doc), `is_default` (exactly one view is the unfiltered/default view at any time -- a shipped seeded default row plus a partial unique index enforce "at most one" at the database, and the API refuses to delete or clear `is_default` on the row currently holding the default with 409 `default_required`. Marking a different view `is_default: true` (POST or PUT) MOVES the default: the incumbent is demoted and the target promoted in one transaction, so it returns 200/201 rather than a conflict and the default is never pinned to the seeded row -- it can be moved, never removed). **Admin-only for every verb, including read** (decision R2-10 "serving/auth dials," same stricter-than-usual read floor as `/repo-credentials`). No generation or serving logic reads this table yet -- Children B/D of #1162's split plan, out of scope here. |
| `/download-retention/state` | GET | ✅ Issue #1453 (PR #1758): ADR-0034 grace-period retention state per tracked item — approaching/past grace period/pinned, three distinct states per the ADR. Viewer+. Optional `?state=` query parameter narrows to exactly one of the closed set `grace`/`pending-purge`/`pinned`; an unrecognized value is 400 `validation_error`. Omitted, it lists all three. Issue #1962: each `grace`-state item also carries a computed `grace_ends_at` (absolute timestamp = `grace_started_at` + the resolved grace period — the row's own `policy_id` when set, else the `default` scope's policy, the SAME authoritative source the retention purge scheduler resolves against). Null/absent for `pending-purge`/`pinned` items and for any row with no resolvable policy. Purely computed — no schema change. |
| `/download-retention/{id}/pin` · `/download-retention/{id}/unpin` | POST | ✅ Issue #1453 (PR #1758): exempt/restore an item from the retention sweep indefinitely. Admin-only. **Re-pin is a 200 no-op (issue #1631):** pinning already-pinned content does not throw and does not overwrite the row's original `pinned_by`/`pinned_at`/`pin_note` — those are preserved from the first pin, not replaced by the repeat call's actor/note. |
| `/download-retention/{id}/purge-now` | POST | ✅ Issue #1453 (PR #1758): skip the remaining grace period deliberately. Admin-only. |
| `/download-retention/dial` | GET, PUT | ✅ Issue #1453 (PR #1758): the separate, operator-configurable manual/ad-hoc download retention dial (ADR-0034 — distinct from any subscription's own grace period). Read Viewer+, write Admin-only. |
...
| `/content-libraries/{libraryId}/folders` · `/content-libraries/{libraryId}/folders/{folderId}` | GET, POST, PATCH, DELETE | ✅ Issue #1389/#1746 (PR #1770): the DB-only virtual organization layer (owner decision 16 — "never on disk," VCSP stays flat) — folder create/rename/move and `PATCH /content-libraries/{libraryId}/items/{itemId}/folder` to (re)assign an item, plus folder delete. Read Viewer+; create/rename/move and item assignment are Operator+, matching decision R2-10's "Operator: ad-hoc downloads, library upload/organize"; folder delete is Admin. See the RBAC reconciliation below. Issue #1812: an unknown folder id referenced anywhere in this family (`parent_folder_id` on create/rename-move, or `folder_id` on item assignment) is uniformly 404 `not_found` — the next endpoint in this family inherits this shape rather than re-deciding it. |
...
| Item CRUD (add/update/remove within a library) | — | ✅ Operation layer shipped (issue #1396, migration 0133): `ContentLibraryItem` model, `content_library_items` table (with the FK migration 0113 shipped without, onto this table, `ON DELETE CASCADE`), and `ContentLibraryItemService` — add/update/remove delegated through the VCSP writer (#1393) so its atomicity/version guarantees hold end to end; UUID reused on update, version incremented, type inferred from file extension. **No HTTP endpoint yet** — the multipart-upload controller surface is its own follow-up, #1826. |
...
Issue #1531's own risk note anticipated this: its "persistent in-app + audit" alert
bullet assumed an injectable `IAlertService` already existed on `main` by the time it
was built. It does not — the whole `/alerts` model above remains 📋 Planned, nothing
in this codebase implements `AlertKinds` or an alert repository/service yet — so
#1531 ships only the HTTP-visible half (409 `disk_admission_denied` on
`POST /downloads/binaries`) and stops there rather than inventing a one-off alert
sink ahead of the model it was meant to plug into. A future `disk_admission_reserve_exceeded`
`kind` is the natural extension once `/alerts` itself is built, tracked as follow-up
scope rather than added speculatively here.
