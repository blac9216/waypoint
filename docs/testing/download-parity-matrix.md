...
(TC-01…TC-30, EP-01…EP-06, ID-01/ID-02) into Waypoint-side contract statements,
...
land — this is a process convention, not CI-enforced.
...
| `out of scope` | The sibling case belongs to the Transfer/air-gap-bundle feature, which is a separate future delivery story (*Transfer & enclave modes*, not yet epic'd) and out of this milestone's scope per #1186's Scope/Non-goals. |
...
implements the `Save-WebFile` resume protocol — `.resume.tmp` merge, the
200-vs-206 response distinction, oversize handling, and 401/403 treated as
non-retryable. `#1411` pins this with concrete tests; every other lane below
has no implementing code on `main` yet and is marked `not yet buildable`.

## TC-01…TC-30: primary test cases

| Case | Waypoint contract | Owning epic | Status |
| --- | --- | --- | --- |
| TC-01 Docker Build | The download-runner image builds cleanly with only air-gap-safe dependencies installed — no `vcf-download-tool` bundled (License & Borrowing Policy), no PowerShell. | #1180 | not yet buildable |
