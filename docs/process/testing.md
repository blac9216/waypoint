...
Path-filtered jobs — `build, test, coverage` (backend/**), `build, test, lint` (frontend/**), `compose config, nginx -t, shellcheck` (deploy/**, scripts/**) — do not report on every PR and therefore cannot be required until the always-report pattern (#232) lands.
