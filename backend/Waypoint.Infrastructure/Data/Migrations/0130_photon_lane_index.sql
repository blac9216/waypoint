...
-- discovered TO EXIST upstream, keyed on that triple (never row-deleted -- a repo
-- that disappears upstream is a discrepancy for a later reconciliation pass to
-- surface, matching this repo's alert-instead-of-drop convention, not this table's
-- own job). has_repodata=false marks a `photon_snapshots`-style directory that DOES
-- exist upstream but has no `repodata/repomd.xml` (research finding #5's open
