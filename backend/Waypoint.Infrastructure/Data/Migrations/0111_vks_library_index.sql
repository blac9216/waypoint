...
CREATE TABLE IF NOT EXISTS vks_library_items (
    ...
    release_line TEXT NULL CONSTRAINT vks_library_items_release_line_check CHECK (release_line IN ('vkr', 'tkg')),
    line_build TEXT NULL,
    ob_build_id TEXT NULL,
    naming_era TEXT NOT NULL CONSTRAINT vks_library_items_naming_era_check CHECK (naming_era IN ('legacy_k8s', 'tkgs_ova', 'vmi_k8s', 'current', 'unparsed')),
    parse_status TEXT NOT NULL CONSTRAINT vks_library_items_parse_status_check CHECK (parse_status IN ('parsed', 'unparsed')),
    etag TEXT NULL,
    sha256 TEXT NULL,
