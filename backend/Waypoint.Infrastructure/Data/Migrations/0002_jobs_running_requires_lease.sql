...
-- make the state unrepresentable rather than merely handled. The dispatcher (#5)
-- already only ever sets state='running' in the same atomic UPDATE that stamps
-- lease_expires_at (see JobQueueRepository.ClaimJobAsync) -- this CHECK is the
-- backstop that keeps that true even if a future code path forgets, or something
-- writes to the row outside the repository (a manual UPDATE, a future migration, an
-- operator console query).
--
...
-- their own lease-tied CHECK -- tracked separately (see the PR body's Deferred
-- Items note) rather than folded in here, since #107 is scoped to 'running' only.
--
